...
\begin{document}
	...
	\section{Neural Function Approximation (\texttt{model.rs})}
	
	Following the Actor-Critic framework (Zhao, Chapter 10), I approximate the policy $\pi$ and the state-value function $v$ using neural networks.
	
	\subsection{Network Architectures}
	I define two separate Multi-Layer Perceptrons (MLPs) with parameters $\theta$ (Actor) and $w$ (Critic). Both networks receive the state vector $s \in [-1, 1]^4$ as input.
	
	\begin{itemize}
		\item \textbf{Actor Network $\pi(a | s, \theta)$}: Maps $s$ to 5 logits, followed by a softmax layer to produce a probability distribution over $\mathcal{A} = \{a_1, \dots, a_5\}$.
		\item \textbf{Critic Network $\hat{v}(s, w)$}: Maps $s$ to a single scalar representing the state-value $v_\pi(s) = \mathbb{E}[G_t | S_t = s]$, which is the expected discounted return from state $s$.
	\end{itemize}
	
	\subsection{The TD Error and Objective Functions}
	To train the networks, I must derive the update signals based on the environment's feedback.
	
	\subsubsection{Original Equations}
	...
	\subsubsection{Derivations for \texttt{Orbit}}
	In my implementation, I use the TD error $\delta_t$ as an unbiased estimate of the \textbf{advantage signal} $A(s, a) \doteq q_\pi(s, a) - v_\pi(s)$, which measures the relative benefit of a specific action compared to the average value of the state. I derive the following specific equations:
	
	\begin{enumerate}
		\item \textbf{TD Error with Termination Logic}: 
		For a transition yielding reward $R_{t+1}$ and next state $S_{t+1}$:
		$$ \delta_t = 
		\begin{cases} 
			R_{t+1} - \hat{v}(S_t, w) & \text{if } \texttt{is\_done = true} \\
			R_{t+1} + \gamma \hat{v}(S_{t+1}, w) - \hat{v}(S_t, w) & \text{otherwise}
		\end{cases} $$
		
		\textit{Note}: If \texttt{is\_truncated = true}, I still bootstrap with $\gamma \hat{v}(S_{t+1}, w)$ as the truncation is a boundary of time, not the underlying MDP state.
		
		\item \textbf{Critic Loss ($L_w$)}: Minimize the Mean Squared Error (MSE) of the TD error: $$ L(w) = \frac{1}{2} \delta_t^2 $$
		
		\item \textbf{Actor Loss ($L_\theta$)}: 
		Using the log-likelihood trick and the TD error as the advantage signal, I define the loss for the \texttt{dfdx} optimizer as:
		$$ L(\theta) = - \ln \pi(A_t | S_t, \theta) \delta_{t, \text{stop\_grad}} $$
